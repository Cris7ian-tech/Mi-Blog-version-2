...
const PokemonDetail = () => {
  ...
  if (!pokemon) return <div className="text-white text-center py-20 font-bold tracking-widest animate-pulse">Sincronizando con la PokeAPI...</div>;
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 bg-[#2D2F39] rounded-[3rem] p-8 md:p-16 border border-[#373943] shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <h1 className="text-5xl font-black text-white capitalize tracking-tighter">{pokemon.name}</h1>
          <div className="flex gap-3 mt-6">
            {pokemon.types.map((t) => (
              <span key={t.type.name} className="px-4 py-1 bg-[#1A1B22] text-[#DE8676] border border-[#DE8676]/30 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">
                {t.type.name}
              </span>
            ))}
          </div>
        ...
        <div className="space-y-6 flex flex-col justify-center">
          <h2 className="text-2xl font-bold text-white mb-4 tracking-tight">Análisis de Estadísticas</h2>
            ...
            {pokemon.stats.map((s: PokemonStat) => (
              <div key={s.stat.name} className="space-y-2">
                ...
                <div className="h-1.5 w-full bg-[#1A1B22] rounded-full overflow-hidden">
                  <div 
                    className="stat-bar h-full bg-[#DE8676] rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(222,134,118,0.5)]"
                    style={{ width: '0%' }}
          ...
          <div className="grid grid-cols-2 gap-4 mt-10 pt-8 border-t border-[#373943]">
            <div className="text-center">
              <p className="text-[#A1A1A1] text-[10px] uppercase font-bold tracking-widest mb-1">Altura</p>
              <p className="text-white text-xl font-black">{pokemon.height / 10} m</p>
            </div>
            <div className="text-center">
              <p className="text-[#A1A1A1] text-[10px] uppercase font-bold tracking-widest mb-1">Peso</p>
              <p className="text-white text-xl font-black">{pokemon.weight / 10} kg</p>
            </div>
          </div>
          ...
          <div className="mt-8 flex gap-4">
            <button 
                ...
                className="flex-1 bg-[#1A1B22] hover:bg-[#DE8676] hover:text-[#1A1B22] text-[#DE8676] py-3 rounded-full text-[10px] font-black uppercase tracking-widest border border-[#DE8676]/20 transition-all"
              >
              Compartir Pokémon
            </button>
          </div>
